...
$user = $_SESSION['user'];
$pass = $_POST['pass'];
$_SESSION['pass_login'] = $pass;    //senas
$passn = $_POST['passn'];
$_SESSION['passn_login'] = $passn;   //naujas
...
if (checkpass($pass, $dbpass))  // senas slaptazodis ivestas geras  
{
  ...
  // ar geras epasto laukas
  if ($ar_pasnn)  // lauku reiksmes geros
  {
    if ($pass != $passn)   // vartotojas kazka keicia
    {
      $dbpass = substr(hash('sha256', $passn), 5, 32);
      $db = mysqli_connect(DB_SERVER, DB_USER, DB_PASS, DB_NAME);
      $sql = "UPDATE " . TBL_USERS . " SET slaptazodis='$dbpass'   WHERE  slapyvardis='$user'";
      ...
      $_SESSION['message'] = "Nieko nekeitėt, paskyra nepakeista";
    }
    $_SESSION['user'] = "";
